...
| `danzhen_panel` | 单针下20 | 单针下20/30 副图 | 副图 | `danzhen_*` |
...
目前项目里关于 `danzhen20` 的最新共识，不再是旧的 `短下20` 口径，而是：

- 正式主信号看副图红线，也就是 `danzhen_long`
- 要求最近 `21` 根里红线至少有一次 `>= 80`
- 当前这根红线下穿 `30`

也就是：

```text
red_line = danzhen_long
signal = 近21根曾经 >= 80，且本根从 >=30 下穿到 <30
```

这个规则已经在策略实现里替代了旧的“短 <= 20 且 长 >= 60”扫描逻辑。
