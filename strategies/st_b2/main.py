#coding:gbk
"""
Stock picker: st_b2 (TongDaXin formula -> xtQMT)
...
def build_candidates(context, t_date):
    ...
    for code in g.universe:
        ...
        zf_ok = False
        if bar_prev["close"] > 0:
            zf_ok = (bar_t["close"] / bar_prev["close"] - 1.0) * 100.0 > DAILY_RETURN_MIN_PCT
        if not zf_ok:
            stats["zf_fail"] += 1
            continue

        if bar_prev["volume"] <= 0 or bar_t["volume"] < bar_prev["volume"] * DAILY_VOL_RATIO_MIN:
            stats["vol_fail"] += 1
            continue

        if j_list[-1] > J_NOW_MAX:
            stats["j_now_fail"] += 1
            continue

        if j_list[-2] >= J_PRE_MAX:
            stats["j_pre_fail"] += 1
            continue
    ...
    _log(
        "filter_stats total={0} bars_short={1} zf_fail={2} vol_fail={3} "
        "j_now_fail={4} j_pre_fail={5} pass={6}".format(
            stats["total"],
            stats["bars_short"],
            stats["zf_fail"],
            stats["vol_fail"],
